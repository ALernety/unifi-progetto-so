...
int socket_open(socket_data socket_input, sa_family_t socket_domain)
{
	...
	case AF_UNIX:
		*socket_input.sfd =
			socket_open_unix(socket_input.sfd, socket_input.user,
					 socket_input.socket_path,
					 socket_input.max_connected_clients);
		break;
	case AF_INET:
		*socket_input.sfd = socket_open_inet(
			socket_input.sfd, socket_input.user,
			socket_input.socket_path, socket_input.port,
			socket_input.max_connected_clients);
		break;
	}
	...
	*sfd = socket(AF_UNIX, SOCKET_TYPE, DEFAULT_PROTOCOL);
	...
	case CLIENT: {
		...
		for (size_t time = 10; time > 0 && connection == -1; time--) {
			...
			sleep(1);
			connection =
				connect(*sfd, socket_address_ptr, socket_len);
		}
